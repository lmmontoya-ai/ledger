...
The name is not an acronym. The world literally is a ledger: an append-only log of events with every state computed as a fold over it, which is what makes any moment of any episode replayable byte-for-byte.
...
The environment's own text is overhead, so it is budgeted and measured (o200k and cl100k; vendor-private tokenizers approximated with stated headroom). A player reads a fixed system block (measured 612/615, byte-identical in every call and therefore cached) plus a state board (measured 342 under template v2's plain wording and markdown section headers, bound ≤360, does not grow) and a one-line-per-turn history with per-class bounds (simple ≤8, executive ≤20, lifecycle ≤36, contract 12+12·jobs+10·pays, message ≤52 at the engine-default 40-token chat cap). The chat cap is a runtime parameter whose stated limit always matches the enforced one; the experiment campaign froze **200** after Phase-0 measurement (message lines then bound ≤ ~212). Typical play runs **~650–950 variable tokens per call**; the full worked-episode prompt at turn 15 measures 1,153. Golden tests enforce every bound under both encodings. The equivalent world rendered as a JSON event log runs 1,500–4,000 tokens per call before any messages, so typical play is 2–5× cheaper.
