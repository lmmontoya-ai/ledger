...
- **2026-08-12, loops 7-8 and DEV-FREEZE 2.** Loop 6 (mid-game openings) backfired: it delayed value instead of adding it (trade-off 0.373, trust 4); reverted, recorded as the LEDGER-adaptation lesson (FORGE's amendment expands the late pie; merely locking existing jobs starves the early one). Loop 7 (pay_cap 20) also backfired: larger extraction partially restores the transfer realignment (trade-off 0.353, trust 19); cap 12 is the balance point. Loop 8 widened the settlement window from the final third to the final half (settle_window 12): IOUs stay outstanding and breakable across twice the game. GATE C PASSES on dev seeds, all bars: trust states 60 (bar 36, quota 18), trade-off 0.459 uniform across phases, separability 100/100/85 and 100/100/97. DEV-FREEZE 2: knobs u=(40,40), anti=0.8, pay_cap=12, settle_window=12, n_arrivals=0; sealed one-shot validation on range 20000:20400.
- **2026-08-12, sealed validation BURNED, loop 6.** The one-shot validation on range 10000:10400 (bank of 12, all opening trade-offs, depths 7-20): trade-off density 0.418 PASS, separability 100/100/87.5 and 100/100/47 PASS at both pass tiers, trust states 28 vs the 36 bar FAIL. The loop-5 recalibration was checked by unseen seeds and came up short; the range is burned per protocol, and the conclusion both banks agree on is that trust-state production needs structure, not bar arithmetic. Loop 6 implements the FORGE-informed lever adapted to LEDGER: per-job `available_from` (mid-game openings). Jobs are on the board from turn 1 but cannot be drawn for or executed before their opening turn (engine predicate `job_available`, schedule feasibility arrival-aware, board status "opens turn N", system-text Openings sentence, all moving together). Deals about unopened jobs are forward contracts whose payments, under the settlement window, are IOUs by construction. Generator opens the 2 highest-joint-value leaf jobs (never the exposure job) at turn 14. Also this date: the state-clone hot spot replaced with an explicit field-wise clone (11.8x per copy, purity suite pins equivalence) and Gate C computes each envelope once; the full gate now runs in ~6 minutes, which is what made a sixth loop affordable. Next sealed range on freeze: 20000:20400.
