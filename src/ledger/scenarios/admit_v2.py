...
from .admit import exposure_probe_state, renege_early_dominates_stall
...
def admit_v2(sc: Scenario, check_probe: bool = True) -> AdmissionV2Report:
    ...
    # 4. exposure probe with the premium band (envelope-computed)
    st, cid_or_reason, _ = exposure_probe_state(sc)
    if st is None:
        rep.admitted = False
